...
> **注意**：乘号运算符 `*` 在 Altair 中**不用于复合图表组合**。它仅在表达式模块中用于数值计算（如 `alt.datum.x * 2`）。详见 [2.4 节](#24-乘号运算符--的特殊说明)。
...
┌─────────────────────────────────────────────────────────────────────────────┐
│                           TopLevelMixin                                        │
│  (顶层混入：to_dict、to_json、to_html、interactive、add_params、resolve_* 等) │
└───────────────────────────────┬───────────────────────────────────────────────┘
                                │
        ┌───────────────────────┼───────────────────────┬───────────────────┐
        │                       │                       │                   │
        ▼                       ▼                       ▼                   ▼
┌───────────────┐      ┌───────────────┐      ┌───────────────────┐  ┌───────────────────┐
│ _EncodingMixin│      │    Chart      │      │  HConcatChart     │  │  VConcatChart      │
│  (encode方法) │      │ LayerChart    │      │  ConcatChart      │  │  RepeatChart       │
│               │      │               │      │  FacetChart       │  │                    │
│  仅提供编码能力 │      │ 继承两个混入   │      │  仅继承 TopLevelMixin │  │  仅继承 TopLevelMixin  │
└───────────────┘      └───────────────┘      └───────────────────┘  └───────────────────┘
```

**正确的继承关系说明**：

| 类名 | 继承 `TopLevelMixin` | 继承 `_EncodingMixin` | 继承来源 |
|------|----------------------|------------------------|----------|
| `Chart` | ✅ 是 | ✅ 是 | `api.py:4001` |
| `LayerChart` | ✅ 是 | ✅ 是 | `api.py:4803` |
| `HConcatChart` | ✅ 是 | ❌ 否 | `api.py:4593` |
| `VConcatChart` | ✅ 是 | ❌ 否 | `api.py:4697` |
| `ConcatChart` | ✅ 是 | ❌ 否 | `api.py:4488` |
| `RepeatChart` | ✅ 是 | ❌ 否 | `api.py:4334` |
| `FacetChart` | ✅ 是 | ❌ 否 | `api.py:4998` |

**设计意图**：
- **Chart 和 LayerChart**：可以在顶层设置 `encoding`，这些编码会被子单元/子层继承
- **拼接类图表 (HConcatChart/VConcatChart/ConcatChart)**：每个子图表独立，不共享顶层编码
- **RepeatChart/FacetChart**：单一子规格，编码在子规格中定义，而非顶层
...
1. **直观的运算符重载**：使用 `+`（分层）、`&`（垂直拼接）、`|`（水平拼接）等熟悉的运算符，配合函数形式 `alt.layer()`、`alt.hconcat()`、`alt.vconcat()`，提供自然的复合图表构建体验。

   > **重要说明**：乘号运算符 `*` **不用于复合图表组合**。它仅在表达式模块中实现，用于 `alt.datum.x * 2` 这样的数值计算。`Chart` 类及其复合子类均未定义 `__mul__` 和 `__rmul__` 方法，尝试使用 `chart1 * chart2` 会抛出 `TypeError`。
...
6. **明确的运算符职责分离**：
   - **复合图表运算符**：`+`、`&`、`|` 定义在 `api.py` 中的图表类中，操作数为 `ChartType`
   - **表达式运算符**：`*`、`/`、`-`、`+` 等定义在 `expr/core.py` 中的 `OperatorMixin` 类中，操作数为表达式对象
